 %% RESPI DATA ANALYSIS FUNCTIONS
 
 classdef data_analysis_respi_functions < matlab.mixin.Copyable
...
%             % time vector.
        ...
        function validParams = checkClassInputs(Bm)
            ...
            if isnumeric(Bm.smoothedRespiration)
                if length(size(Bm.smoothedRespiration)) == 2
                    if size(Bm.smoothedRespiration,1) ~= 1
                        Bm.smoothedRespiration = Bm.smoothedRespiration';
                    end
                    respCheck=1;
                else
                    respCheck=0;
...
%             breathIndx = sort([putativePeaks putativeTroughs]);
%             breathStatus = sampleStatus(breathIndx);
%             Bm.statuses = breathStatus;
            ...
            if strcmp(Bm.dataType,'humanBB')
                ...
                if (~isempty(putativeTroughs))&&(putativeTroughs(1)>putativePeaks(1))
        ...
        function Bm = findInhaleAndExhaleOffsets(Bm, verbose )
            ...
            [inhaleOffs,exhaleOffs]=data_analysis_respi_offsets_modif1(thisResp, ...
        ...
        function Bm = findBreathAndPauseDurations(Bm)
            ...
            % Only valid for analyzing airflow data.
            [inhaleDurs, exhaleDurs] = data_analysis_respi_breath_duration(Bm);
...
%         function Bm = findInspiratoryExpiratoryTimes(Bm, verbose)
%             % estimate inspiratory and expiratory times from onset to
%             % offset (in milliseconds, since the sampling rate is at 1000
%             % samples per second, it leads to 1 sample per millisecond)
%             if nargin < 2
%                 verbose = 0;
%             end
%             
...
%             
%             if verbose==1
%                 disp('to calculate inspiratory and expiratory time');
%             end
%             
%             inspiratoryTime=(Bm.inhaleOffset-Bm.inhaleOnsets)/Bm.srate;
%             expiratoryTime=(Bm.exhaleOffset-Bm.exhaleOnsets)/Bm.srate;
%             
%             % sometimes no trough of exhale at last breath
%            % nTroughs=length(Bm.exhaleTroughs);
%            % timeToTroughs=(Bm.exhaleTroughs(1:nTroughs)-Bm.exhaleOnsets(1:nTroughs))/Bm.srate;
%             
%             Bm.inhaleTimeToPeak=inspiratoryTime;
%             Bm.exhaleTimeToTrough=expiratoryTime;
%         end
       
        
        function Bm = getRespiFeaturesCalculations( Bm, verbose )
        ...
        function Bm = checkFeatureEstimations(Bm)
            ...
            otherSignalCompleteFeatureSet={
                ...
                'exhaleOnsets';'inhaleOffsets';'exhaleOffsets';
                'inhaleDurations';'exhaleDurations';'secondaryFeatures'};
        ...
        function Bm = estimateAllRespiFeatures( Bm, zScore, ...
            ...
            elseif strcmp(Bm.dataType,'humanBB') || ...
                ...
                Bm.findInhaleAndExhaleVolumes(verbose);
                Bm.findBreathAndPauseDurations();
                % Bm.findInspiratoryExpiratoryTimes(verbose);
                Bm.getRespiFeaturesCalculations(verbose);
            end
        ...
        function fig = plotFeatures(Bm, annotate, sizeData)
            ...
            fig = plot_data_analysis_respi(Bm, annotate, sizeData);
        end
