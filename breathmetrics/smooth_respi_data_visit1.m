...
commonPath = fullfile('\\', 'svfas5.epfl.ch', 'Sandi-Lab', 'Catherine', ...
    'XP VIBES', 'STUDY 1', 'ANALYSIS', 'CLEAN', 'PHYSIO', 'CUTDATA', 'V1');
folderNames = {'1 BASELINE EYES CLOSED', '2 BASELINE EYES OPENED', ...
    '3 ANTICIPATION'};
dataFields = {'eyesClosedBaseline', 'eyesOpenedBaseline', 'anticipationTSST'};
...
for iFolders = 1:length(folderNames)
    ...
    resultsDir = fullfile('\\', 'svfas5.epfl.ch', 'Sandi-Lab', ...
        ...
        'RESPIRATION', 'V1', 'WITHOUT OVERSATURATIONS', folderNames{iFolders}); % CHANGE POTENTIAL OVERSATURATIONS TO WITHOUT OVERSATURATIONS WHEN DONE WITH THE FORMER VERSION OF THE INTERP FUNCTION
    ...
    for iFiles = 1:length(fileNames)
        ...
        try
            [interpSmoothData, ~, NSat, NNaN, NInterp, sampleStatus] = interpol_1d_data_no_oversaturation(denoisedData, [-4.89 1.92], 0); % potential oversaturation >> interpol_1d_data_symetric_points; without oversaturation>> interpol_1d_data_no_oversaturation_peaks_window_length
            disp(NSat);
            disp(NNaN);
            disp(NInterp);
...
%            ylabel('Respiratory Flow');
        ...
        cleanedRespiData = bmObj.baselineCorrectedRespiration;
         save([resultsDir,filesep,'s_',respDataFileName],...
             'cleanedRespiData', 'sampleStatus');
        
    end
end
resultsDirERRORS = fullfile('\\', 'svfas5.epfl.ch', 'Sandi-Lab', ...
    'Catherine','XP VIBES','STUDY 1', 'ANALYSIS','CLEAN','PHYSIO', ...
    'RESPIRATION', 'V1', 'WITHOUT OVERSATURATIONS'); % change from POTENTIAL OVERSATURATIONS to WITHOUT OVERSATURATIONS when needed
...
disp(['the error file has successfully been uploaded in ',...
    'Catherine > XP VIBES > STUDY 1 > ANALYSIS > CLEAN > PHYSIO > RESPIRATION > V1 > WITHOUT OVERSATURATIONS']);
%% done