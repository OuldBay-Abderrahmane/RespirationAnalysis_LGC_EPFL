%% CUT VISIT 2 automatized cutting tool for visit2 all phases
...
for iFiles = 76:length(fileNames)
    currentDataFileName = fileNames(iFiles, :);
    rawData = load(fullfile(externalDriveDir, currentDataFileName));
    ...
    for iFolders = 1:length(events(:,2))
...
%          else
             % in case of errors
%              errors(rowErrors, :) = strcat('error in V',...
%                  num2str(currentDataFileName(iFiles,2:4)), 'for event ', ...
%                  eventPhaseName, ' of visit2, keyCode = ', keyCode);
%              rowErrors = rowErrors + 1;
        end
